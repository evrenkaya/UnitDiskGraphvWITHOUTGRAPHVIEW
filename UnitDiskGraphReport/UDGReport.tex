...
%A disk graph, G is defined as a geometric graph that contains $n$ disks each with radius $r$ in the Euclidean plane. The center point of every disk is considered a vertex and two vertices, $u, v$ have a straight-line edge between them if the Euclidean distance between them, denoted by $|uv|$ is at most $r$. It is when $r=1$, that G is called a Unit Disk Graph.
\newpage
...
%Each vertex has x and y coordinates between 0 and 1 that are both of the double data type. A new set of $n$ randomly placed vertices is created by using Java's built in Random class, more specifically, each vertex is assigned random x and y coordinates from the method named nextDouble() within the Random class.
